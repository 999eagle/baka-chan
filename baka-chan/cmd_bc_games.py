...
async def cmd_points(message, args):
	...
	if len(args) == 0:
		...
		user = args[0]
	if user != None:
		points = str(globals.data_coins.getcoins(server, user))
		await send_message(message.channel, '{1} has {2} {0}'.format(globals.config.currency_name, user, points))
	else:
		if len(args) == 3 and args[0] in ['spawn', 'give', 'despawn'] and is_usermention(args[1]) and is_int(args[2]):
			...
			if args[0] == 'spawn':
				if not sender_has_permission(message, DataPermissions.Permission.coins_spawn):
					...
					return
				if int(args[2]) > 0:
					points = globals.data_coins.getcoins(server, user) + int(args[2])
					globals.data_coins.setcoins(server, user, points)
					await send_message(message.channel, '{1} received {2} {0} and now has {3} {0}'.format(globals.config.currency_name, user, args[2], str(points)))
				else:
				...
				if int(args[2]) > 0:
					...
					if points >= 0:
						globals.data_coins.setcoins(server, user, points)
						await send_message(message.channel, '{1} lost {2} {0} and now has {3} {0}'.format(globals.config.currency_name, user, args[2], str(points)))
					else:
						await send_message(message.channel, '{1} only has {2} {0}'.format(globals.config.currency_name, user, str(globals.data_coins.getcoins(server, user))))
				else:
				...
				if int(args[2]) > 0:
					points = globals.data_coins.getcoins(server, user) + int(args[2])
					points2 = globals.data_coins.getcoins(server, user2) - int(args[2])
					if points2 >= 0:
						globals.data_coins.setcoins(server, user, points)
						globals.data_coins.setcoins(server, user2, points2)
						await send_message(message.channel, '{1} received {2} {0} from {4} and now has {3} {0}'.format(globals.config.currency_name, user, args[2], str(points), user2))
					else:
						await send_message(message.channel, '{4} wanted to give {2} {0} to {1}, but only has {5} {0}'.format(globals.config.currency_name, user, args[2], str(points), user2, str(globals.data_coins.getcoins(server, user2))))
				else:
					await send_message(message.channel, '{2} wanted to steal {0} from {1}!'.format(globals.config.currency_name, user, user2))
		else:
...
async def cmd_slots(message, args):
	...
	if len(args) == 0 or not is_int(args[0]):
		...
		if spending <= 0:
			...
			return
		user = '<@' + message.author.id + '>'
		points = globals.data_coins.getcoins(server, user)
		if spending > points:
			...
			points += spending * win
			globals.data_coins.setcoins(server, user, int(points))
			await send_message(message.channel, text)